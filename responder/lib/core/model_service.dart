...
/// OFFLINE_MODEL_PLAN.md).
///
/// Non-disruptive by design: [ensureLatest] runs in the background after
/// login and fails silently; the "Assess vitals" feature simply stays hidden
/// until a verified model file exists on disk. Once downloaded, inference is
/// fully offline.
class ModelService {
  ModelService._();
  static final ModelService I = ModelService._();
  ...
  Future<void> ensureLatest() async {
    ...
    try {
      final res = await Api.I.dio.get('/models/$_name');
  ...
  /// the eval showed too few inputs gives unsafe answers.
